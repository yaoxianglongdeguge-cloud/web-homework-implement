...
static struct pkt* A_packet=NULL;
...
void A_packet_init(struct msg message)
{
  ...
  A_packet->acknum=0;
  A_packet->seqnum=A_ou_seq;
  A_packet->checksum=gene_check(*A_packet);
}
  ...
  struct pkt packet;
{
  ...
  if(checknum!=packet.checksum)
  {
   ...
   if(packet.acknum==A_ex_ack)
   {
    stoptimer(0);
    A_ou_seq=(A_ou_seq+1)%2;
    A_ex_ack=A_ou_seq;
   }
   else
   {
...
void A_init()
{
     ...
     A_packet=(struct pkt*)malloc(sizeof(struct pkt));
